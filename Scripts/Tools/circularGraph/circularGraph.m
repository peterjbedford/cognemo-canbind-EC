  ...
  methods
    function this = circularGraph(adjacencyMatrix,varargin)
      ...
      for i = 1:length(adjacencyMatrix)
        ...
        %MarkerSize
        markersize = get(gca,'Position'); units = get(gcf,'Units');
        % markersize = get(gca,'Position'); units = get(gcf,'Units');
        
        if units=='pixels'
            markersize = 20*markersize(3)/length(adjacencyMatrix);
            % markersize = 5.4*markersize(3)/length(adjacencyMatrix);
        elseif units=='inches'
            markersize = 72*20*markersize(3)/length(adjacencyMatrix);
            % markersize = 72*5.4*markersize(3)/length(adjacencyMatrix);
        end
        this.Node(i).NodeMarker.MarkerSize = markersize;
        %this.Node(i).NodeMarker.Color = [0 0 1];
      end
